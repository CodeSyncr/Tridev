...
class Request implements RequestOrResponse {
  ...
  ///
  /// You should typically not manipulate this property's [HttpRequest.response]. By default, Tridev controls
  /// the response through its [Controller]s.
  ///
  /// If you wish to respond to a request manually - and prohibit Tridev from responding to the request - you must
  ...
  ///
  /// Once this method has executed, the [Request] is no longer valid. All headers from [tridevResponse] are
  /// added to the HTTP response. If [tridevResponse] has a [Response.body], this request will attempt to encode the body data according to the
  /// Content-Type in the [tridevResponse]'s [Response.headers].
  ///
  Future respond(Response tridevResponse) {
    respondDate = DateTime.now().toUtc();

    final modifiers = _responseModifiers;
    _responseModifiers = null;
    modifiers?.forEach((modifier) {
      modifier(tridevResponse);
    });

    _Reference<String> compressionType = _Reference(null);
    var body = tridevResponse.body;
    if (body is! Stream) {
      ...
      body = _responseBodyBytes(tridevResponse, compressionType);
    }

    response.statusCode = tridevResponse.statusCode!;
    tridevResponse.headers.forEach((k, v) {
      response.headers.add(k, v as Object);
    });

    if (tridevResponse.cachePolicy != null) {
      response.headers.add(HttpHeaders.cacheControlHeader,
          tridevResponse.cachePolicy!.headerValue);
    }
    ...
    response.headers.add(
        HttpHeaders.contentTypeHeader, tridevResponse.contentType.toString());
    ...
    if (body is List<int>) {
      ...
      final bodyStream = _responseBodyStream(tridevResponse, compressionType);
      if (compressionType.value != null) {
        response.headers
            .add(HttpHeaders.contentEncodingHeader, compressionType.value!);
      }
      response.headers.add(HttpHeaders.transferEncodingHeader, "chunked");
      response.bufferOutput = tridevResponse.bufferOutput;

...
/// It is unlikely that you need to use this class directly - the `tridev serve` command creates an application object
/// on your behalf.
class Application<T extends ApplicationChannel?> {
  ...
  ///
  /// This logger's name will appear as 'tridev'.
  Logger logger = Logger("tridev");
  ...
  Future start({int numberOfInstances = 1, bool consoleLogging = false}) async {
    if (server != null || supervisors.isNotEmpty) {
      throw StateError(
          "Application error. Cannot invoke 'start' on already running Tridev application.");
    }
  ...
  Future startOnCurrentIsolate() async {
    if (server != null || supervisors.isNotEmpty) {
      throw StateError(
          "Application error. Cannot invoke 'test' on already running Tridev application.");
    }
  ...
  ///
  /// This method is called by the `tridev document` CLI.

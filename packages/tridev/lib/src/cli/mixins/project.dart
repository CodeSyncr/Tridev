...
import 'package:tridev/src/cli/scripts/get_channel_type.dart';
...
abstract class CLIProject implements CLICommand {
  ...
  Version? get projectVersion {
    if (_projectVersion == null) {
      ...
      final projectVersion =
          lockFileContents["packages"]["tridev"]["version"] as String;
  ...
  void preProcess() {
    try {
      if (!isMachineOutput) {
        displayInfo("Tridev project version: $projectVersion");
      }

      if (projectVersion?.major != toolVersion!.major) {
        throw CLIException(
            "CLI version is incompatible with project tridev version.",
            instructions: [
              "Install tridev@${projectVersion?.toString()} or upgrade your project to tridev${toolVersion.toString()}."
            ]);
      }
    } on CLIException {
      rethrow;

...
/// Used internally.
class CLITemplateCreator extends CLICommand with CLITridevGlobal {
  CLITemplateCreator() {
  ...
  Future<int> handle() async {
    if (projectName == null) {
      printHelp(parentCommandName: "tridev");
      return 1;
    }
    ...
    displayProgress("See more templates with 'tridev create list-templates'");
    copyProjectFiles(destDirectory, templateSourceDirectory, projectName!);

    createProjectSpecificFiles(destDirectory.path);
    try {
      final tridevLocation = tridevPackageRef!.resolve()!.location;
  ...
  Uri _packageUri(Directory tridevLocation, String packageDir) {
    return Directory(join(tridevLocation.path, '..', packageDir)).uri;
  }
  ...
  String get detailedDescription {
    return "This command will use a template from the tridev package determined by either "
  ...
  String get description {
    return "Creates Tridev applications from templates.";
  }
...
class CLITemplateList extends CLICommand with CLITridevGlobal {
  @override
  Future<int> handle() async {
  ...
  @override
  String get description {
    return "List Tridev application templates.";
  }
...
class CLITridevGlobal {
  PubCache pub = PubCache();

  PackageRef? get tridevPackageRef {
    var apps = pub.getGlobalApplications();
    if (apps.isEmpty) {
      return null;
    }
    return apps
        .firstWhere((app) => app.name == "tridev")
        .getDefiningPackageRef();
  }

  Uri? get templateDirectory {
    return tridevPackageRef?.resolve()?.location.uri.resolve("templates/");
  }

  ...
  Future<int> setupTestEnvironment() async {
    if (!hasPSQLCLI) {
      displayError(
          ...
          "you must do so to run tests in a Tridev application. For macOS users, "
    ...
    var commands = [
      ...
      "grant all on database tridev_test_db to dart;"
    ];
    ...
    for (var cmd in commands) {
      ...
      if (output.contains("CREATE DATABASE")) {
        displayProgress("Successfully created database tridev_test_db.");
      } else if (output.contains("CREATE ROLE")) {
        displayProgress(
            "Successfully created role 'tridev_test_user' with createdb permissions.");
      } else if (output.contains("ALTER ROLE")) {
        displayProgress(
            "Successfully set user 'tridev_test_user' password to 'tridev!'.");
      } else if (output.contains("GRANT")) {
        displayProgress(
            "Successfully granted all privileges to database tridev_test_db to user 'tridev_test_user'.");
      }

      if (output.contains("database \"tridev_test_db\" already exists")) {
        displayProgress("Database tridev_test_db already exists, continuing.");
      } else if (output.contains("role \"tridev_test_user\" already exists")) {
        displayProgress("User 'tridev_test_user' already exists, continuing.");
    ...
    displayInfo(
        "Congratulations! Tridev applications can now be tested locally.");
